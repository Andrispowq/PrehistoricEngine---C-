...
namespace Prehistoric
{
	...
	{
	public:
		GLTexture(uint32_t width, uint32_t height, ImageFormat format = R8G8B8A8_LINEAR, ImageType type = TEXTURE_2D, bool multisample = false, bool is_depth = false);
		GLTexture(uint32_t id, uint32_t width, uint32_t height, ImageFormat format = R8G8B8A8_LINEAR, ImageType type = TEXTURE_2D);
		GLTexture(uint32_t width, uint32_t height, uint32_t depth, ImageFormat format = R8G8B8A8_LINEAR, ImageType type = TEXTURE_2D, bool multisample = false, bool is_depth = false);
		GLTexture(uint32_t id, uint32_t width, uint32_t height, uint32_t depth, ImageFormat format = R8G8B8A8_LINEAR, ImageType type = TEXTURE_2D);
		GLTexture();
		...
		inline bool isDepth() const { return is_depth; }
	public:
		...
		static Texture* StorageTrue3D(uint32_t width, uint32_t height, uint32_t depth, ImageFormat format = R8G8B8A8_LINEAR, SamplerFilter filter = Trilinear, TextureWrapMode wrapMode = Repeat, bool generate_mipmaps = true);
