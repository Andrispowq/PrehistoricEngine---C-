...
namespace Prehistoric
{
	...
	void GLRayTracingPipeline::BindPipeline(CommandBuffer* buffer) const
	{
		...
		for (const auto& binding : textureBindingTable)
		{
			...
			if (binding.second.second == (uint32_t)RayTracingAccessFlags::READ_ONLY)
				access = GL_READ_ONLY;
			else if (binding.second.second == (uint32_t)RayTracingAccessFlags::WRITE_ONLY)
				access = GL_WRITE_ONLY;
			else if (binding.second.second == (uint32_t)RayTracingAccessFlags::READ_WRITE)
