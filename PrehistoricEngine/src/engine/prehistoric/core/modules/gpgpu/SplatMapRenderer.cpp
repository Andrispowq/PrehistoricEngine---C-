...
namespace Prehistoric
{
	SplatMapRenderer::SplatMapRenderer(Window* window, AssembledAssetManager* manager, uint32_t N)
	{
		...
		{
			pipeline = manager->storePipeline(new GLComputePipeline(window, man, man->loadShader(ShaderName::GPGPUSplat).value()));
		...
		if (__FrameworkConfig.api == OpenGL)
		{
			...
			glPipe->addTextureBinding(0, splatmap.pointer, (AccessMask)ComputeAccessFlags::WRITE_ONLY);
		}
		else if (__FrameworkConfig.api == Vulkan)
		{
